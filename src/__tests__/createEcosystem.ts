...
export async function createEcosystem(
  ...
  return {
    ...
    cleanup: async () => {
      kernel.container.get(UsersCollection).deleteMany({});
      kernel.container.get(PermissionsCollection).deleteMany({});
      kernel.container.get(SessionsCollection).deleteMany({});
    },
    teardown: () => {
